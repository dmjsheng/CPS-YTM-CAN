...
/*!
 * @file uart_driver.h
 * @brief UART driver public interface — types, configuration structures, and API.
 */
...
/*!
 * @brief UART transfer engine selection.
 *
 * | Value                 | Description                              |
 * |-----------------------|------------------------------------------|
 * | UART_USING_DMA        | Use DMA engine for data movement.        |
 * | UART_USING_INTERRUPTS | Use CPU interrupts for data movement.    |
 */
typedef enum
{
...
/*!
 * @brief Number of data bits per UART character.
 */
typedef enum
{
...
/*!
 * @brief Idle-line detection length (in character times).
 */
typedef enum
{
...
/*!
 * @brief CTS signal source for transmit flow control.
 */
typedef enum
{
...
/*!
 * @brief CTS sampling point for transmit flow control.
 */
typedef enum
{
...
/*!
 * @brief RX FIFO watermark at which RTS is de-asserted.
 */
typedef enum
{
...
/*!
 * @brief Polarity of the transmit RTS output signal.
 */
typedef enum
{
...
/*!
 * @brief TX line direction in one-wire (half-duplex) mode.
 */
typedef enum
{
 ...
 *
 * The caller provides memory for this structure during initialization;
 * the driver does not statically allocate memory.
 *
 * @warning Do not modify fields of this structure directly from application
 * code — use the UART_DRV_* API functions instead.
 */
typedef struct
{
 ...
 *
 * Passed to UART_DRV_Init() to configure the peripheral. Use
 * UART_DRV_GetDefaultConfig() to populate with safe defaults (9600 8N1).
 */
typedef struct
{
...
#if FEATURE_UART_HAS_MODEM_SUPPORT
/*!
 * @brief Transmit CTS (Clear To Send) configuration.
 */
typedef struct
{
    bool txCtsEnable;
...
/*!
 * @brief Receive RTS (Request To Send) configuration.
 */
typedef struct
{
    bool rxRtsEnbale;
...
/*!
 * @brief Transmit RTS configuration.
 */
typedef struct
{
    bool txRtsEnable;
...
#if defined(FEATURE_UART_HAS_ONE_WIRE_MODE_SUPPORT) && (FEATURE_UART_HAS_ONE_WIRE_MODE_SUPPORT == 1U) 
/*!
 * @brief One-wire (half-duplex) mode configuration.
 */
typedef struct
{
 ...
 * @retval STATUS_BUSY the receive is still in progress. @a bytesRemaining will be
 *     filled with the number of bytes that are yet to be received.
...
#if FEATURE_UART_HAS_MODEM_SUPPORT
/*!
 * @brief Configure transmit CTS (Clear To Send) flow control.
 *
...
/*!
 * @brief Configure receive RTS (Request To Send) flow control.
 *
 * @param[in] instance         UART instance index.
...
/*!
 * @brief Configure transmit RTS polarity and enable.
 *
 * @param[in] instance         UART instance index.
...
#if defined(FEATURE_UART_HAS_ONE_WIRE_MODE_SUPPORT) && (FEATURE_UART_HAS_ONE_WIRE_MODE_SUPPORT == 1U) 
/*!
 * @brief Configure single-wire (one-wire) half-duplex mode.
 *
 * @param[in] instance           UART instance index.
 ...
 *
 * @param[in] instance        UART instance index.
 * @param[in] txLinDerection  TX line direction selector.
 */
